 ...
 */
class FreestrokeModelEntries extends JModelList {
	protected $meetsid;
	...
	protected function getListQuery() {
		...
		$query->from ( '`#__freestroke_entries` AS a' );
		$query->join ( 'INNER', '#__freestroke_members AS mb ON mb.id = a.members_id' );
		$query->join ( 'INNER', '#__freestroke_swimstyles AS ss ON ss.id = a.swimstyles_id' );
		$query->where ( 'a.meets_id = ' . $db->Quote($id) );
		$query->order ( array (
				'a.eventnumber ASC', 'mb.lastname', 'mb.birthdate'
		) );
