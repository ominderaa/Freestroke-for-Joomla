...
<div class="items">
		...
		foreach ($this->meetsessions as $meetsession) {
			...
			<hr />
			<h2>Opstelling <?php if(count($this->meetsessions) > 1) { echo  $meetsession->name; }?></h2>
			
			<?php if($canEdit && !$isModal) { ?>
				<div id="freestroke-toolbar" class="freestroke-toolbar">
					<a class="fs-button"
						href="<?php echo JRoute::_('index.php?option=com_freestroke&task=meetsession.edit&layout=edit'.'&return=meetentries&id='.$meetsession->id);?>"
						rel="{size: {x:650,y:450} }"
						title="<?php echo JText::_('COM_FREESTROKE_EDIT_ITEM');?>">
						<?php echo JText::_('COM_FREESTROKE_EDIT_ITEM'); ?>
					</a>
				</div>
			<?php } ?>
