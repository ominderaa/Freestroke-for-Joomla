 ...
 * member Table class
 */
class FreestrokeTableentrie extends JTable {
	...
	public function __construct(&$db) {
		...
		parent::__construct ( '#__freestroke_entries', 'id', $db );
	}
	...
	public function bind($array, $ignore = '') {
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_freestroke.entrie.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_freestroke/access.xml',
				"/access/section[@name='member']/"
			);
			$default_actions = JAccess::getAssetRules('com_freestroke.entrie.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName() {
		$k = $this->_tbl_key;
		return 'com_freestroke.entry.' . ( int ) $this->$k;
	}
