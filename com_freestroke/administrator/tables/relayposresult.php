 ...
 * member Table class
 */
class FreestrokeTablerelayposresult extends JTable {
	...
	public function __construct(&$db) {
		...
		parent::__construct ( '#__freestroke_relayposresults', 'id', $db );
	}
	...
	public function bind($array, $ignore = '') {
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_freestroke.relayposresult.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
					JPATH_ADMINISTRATOR . '/components/com_freestroke/access.xml',
					"/access/section[@name='member']/"
					);
			$default_actions = JAccess::getAssetRules('com_freestroke.relayposresult.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName() {
		$k = $this->_tbl_key;
		return 'com_freestroke.relayposentry.' . ( int ) $this->$k;
	}
