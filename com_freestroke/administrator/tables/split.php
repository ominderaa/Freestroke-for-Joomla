 ...
 * member Table class
 */
class FreestrokeTablesplit extends JTable {
	...
	public function __construct(&$db) {
		...
		parent::__construct ( '#__freestroke_splits', 'id', $db );
	}
	...
	public function bind($array, $ignore = '') {
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_freestroke.split.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
					JPATH_ADMINISTRATOR . '/components/com_freestroke/access.xml',
					"/access/section[@name='member']/"
					);
			$default_actions = JAccess::getAssetRules('com_freestroke.split.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName() {
		$k = $this->_tbl_key;
		return 'com_freestroke.split.' . ( int ) $this->$k;
	}
